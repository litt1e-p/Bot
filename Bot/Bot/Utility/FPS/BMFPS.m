...
@implementation BMFPS

static BMFPS *shareInstance = nil;
...
+ (BMFPS *)shareInstance
{
    ...
    dispatch_once(&onceToken, ^{
        shareInstance = [[BMFPS alloc] init];
    });
...
- (void)start
{
    ...
    NSLog(@"FPSStatus has been stoped.Because it would make no sense for the simulator device.");
    return;
#endif
    
#if !DEBUG
    NSLog(@"Just use FPSStatus in the DEBUG mode.Don't use it for the Relese mode.");
    return;
#endif
    if (self.isStart) {
        return;
